...
int main()
{
	...
	sun.set_scale(glm::vec3(0.3f));
	sun.set_spin(sun_spin);
	//sun.set_orbit({1.5f, glm::radians(-66.0f), glm::two_pi<float>() / 1.3f});
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// nodes.
		using namespace std;
		stack <CelestialBodyRef> stack_planets;

		CelestialBodyRef cBodyRef;

		cBodyRef.body = &sun; //root node
		cBodyRef.parent_transform = sun.render(animation_delta_time_us, camera.GetWorldToClipMatrix(), glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)), show_basis);

		stack_planets.push(cBodyRef);

		//CelestialBody current_body;
		glm::mat4 parent_transform;

		std::vector <CelestialBody*> current_children;

		while (!stack_planets.empty()) {
			cBodyRef = stack_planets.top();
			stack_planets.pop();
			parent_transform = cBodyRef.parent_transform;
			CelestialBody* actual_body = cBodyRef.body;
			current_children = actual_body->get_children();

			for (std::size_t i = 0; i < current_children.size(); ++i) {
				cBodyRef.body = current_children[i]; //child
				cBodyRef.parent_transform = current_children[i]->render(animation_delta_time_us, camera.GetWorldToClipMatrix(), parent_transform, show_basis);
				stack_planets.push(cBodyRef);
			}
		}
